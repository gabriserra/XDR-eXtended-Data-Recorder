function [suddens, turns, static_crashes, dynamic_crashes,dynamic_crashes2] = main_checker(datain,n_datain)
...
%     datain = filler(datain);                                              
    ...
    dynamic_crashes2 = Dynamic_crash_checker2(accx);
...
%     
    ...
    %remove turns and dynamics and suddens from static_crashes
    proximity_margin = 25;
        ...
        for i=1:1:size(static_crashes,1)
            ...
            to_remove = find(dynamic_crashes(:,1) < upper & dynamic_crashes(:,1) > lower);
            dynamic_crashes(to_remove,:) = [];
        end
  


   %remove dynamic crashes from sudden braking
%     proximity_margin = 100;
%         
...
%                 to_remove = find(dynamic_crashes(:,1) < upper );
%                 dynamic_crashes(to_remove,:) = [];
%             end
%         end
    
    %remove detected suddens,turns near to dynamic crashes
    proximity_margin = 60;
